...
server <- function(input,output){
  ...
  #trout filtered reactive dataframe ----
  trout_filtered_df <- reactive ({ # have to make a reactive dataframe
    clean_trout %>% 
  ...
  #penguin filtered island reactive dataframe ----
  island_df <- reactive ({ # have to make a reactive dataframe
    penguins %>% 
      filter(island %in% c(input$Island_type_input))
  })
  
  # render the penguin flipper shistogram
  output$penguin_histogram_output <- renderPlot({
    ggplot(na.omit(island_df()), aes(x = flipper_length_mm, fill = species)) +
      geom_histogram(alpha = 0.6, position = "identity", bins = input$penguin_slider_input) +
